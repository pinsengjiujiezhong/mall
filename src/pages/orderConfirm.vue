<template>
  <div class="order-confirm">
        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" v-for="(item, index) in addressList" :key="index" @click="addressCheck(index)" :class="addr_checked == index ? 'addr-checked' : ''">
                ...
                <div class="action">
                  <a href="javascript:;" class="fl">
                    <svg class="icon icon-del" @click.stop="removeAddress(item.id)"><use xlink:href="#icon-del"></use></svg>
                  </a>
                  <a href="javascript:;" class="fr">
                    <svg class="icon icon-edit" @click.stop="editAddress(item.id)"><use xlink:href="#icon-edit"></use></svg>
                  </a>
                </div>
              </div>
          ...
          <div class="item-good">
            <h2>商品</h2>
            <ul v-if="this.cartList.length">
              <li v-for="(item, index) in cartList" :key="index">
                <div class="good-name">
                  <img :src="item.productMainImage" alt="">
                  <span>{{item.productName + item.productSubtitle}}</span>
                </div>
                <div class="good-price">{{item.productTotalPrice}}元x{{item.quantity}}</div>
          ...
          <div class="detail">
            <div class="item">
              ...
              <span class="item-val">{{cartSum}}件</span>
            </div>
            <div class="item">
    ...
    <modal
      ...
      :showModalChance="showModalChance"
    >
      <template v-slot:body>
        <div class="edit-wrap">
          <div class="item">
            <input type="text" class="input" v-model="address.receiverName" placeholder="姓名">
            <input type="text" class="input" v-model="address.receiverMobile" placeholder="手机号">
          </div>
          <div class="item">
            <select name="province" v-model="address.receiverProvince">
            ...
            <select name="district" v-model="address.receiverDistrict">
          ...
          <div class="item">
            <textarea name="street" v-model="address.receiverAddress"></textarea>
          </div>
          <div class="item">
            <input type="text" class="input" placeholder="邮编" v-model="address.receiverZip">
          </div>
        </div>
      </template>
    </modal>
...
export default{
  ...
  methods:{
    ...
    showModalChance(){
      console.log('已经调用了showModalChance')
    ...
    // 订单提交
    orderSubmit(){
      let item = this.addressList[this.addr_checked]
      if (!item) {
        this.$message.error('当前未选中位置')
      }
    ...
    addrSubmit() {
      ...
      const path = this.isEdit ? `/shippings/${this.editId}` : '/shippings'
      const text = this.isEdit ? '编辑' : '新建'
      this.axios[method](path, {
        ...this.address
      }).then(() => {
        this.showEditModal = false
        this.$message.success('地址' + text + '成功')
        this.getAddressList()
      }).catch(() => {
        this.$message.error(text + '错误, 请检查数据后再次提交')
      })
    },
    editAddress(id) {
      this.isEdit = true
      console.log('进来了editAddress', id)
      this.axios.get(`/shippings/${id}`).then((res) => {
        this.showEditModal = true
        Object.keys(this.address).forEach(key=>{this.address[key]=res[key]})
        this.editId = res.id
    ...
    removeAddress(id) {
      console.log('进入删除了')
      this.axios.delete(`/shippings/${id}`).then((res) => {
        console.log(res)
